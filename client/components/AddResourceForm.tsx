...
const AddResourceForm = forwardRef<AddResourceFormHandle, AddResourceFormProps>(
  function AddResourceForm({ mode = "new", resourceData, isCollapsed: isCollapsedProp }, ref) {
    ...
    return (
      <div className={isControlled ? "" : "bg-white rounded-lg shadow-sm border border-gray-200 p-6"}>
        {/* Candidate Details Section - Collapsible (only show header when not controlled by parent) */}
        {!isControlled && (
          <>
            <button
              type="button"
