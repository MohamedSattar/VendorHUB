...
/**
 * Get Engagements content from different OData endpoint
 * GET /api/odata/engagements
 */
export const handleGetEngagements: RequestHandler = async (req, res) => {
  try {
    const ENGAGEMENTS_BASE_URL = "https://ecavendorhub.powerappsportals.com/_api";

    const url =
      `${ENGAGEMENTS_BASE_URL}/prmtk_engagements?` +
